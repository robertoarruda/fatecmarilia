...
class Administrativo_Cargo {
    ...
    public function listarLimitado($p1, $p2 = 0) {
        $strSQL = "
            SELECT administrativos_funcionarios_funcionariosLogin_cpf, cargos_codigo
            FROM $this->tabela
        ...
        while ($reg = $stmt->fetch()) {
            $obj = new Administrativo_Cargo();
            $obj = $obj->consultar($reg['administrativos_funcionarios_funcionariosLogin_cpf'], $reg['cargos_codigo']);
            $res[] = $obj;
        }
        $stmt = NULL;
        return $res;
    ...
    public function listarTudoFuncionario($p) {
        $strSQL = "
            SELECT administrativos_funcionarios_funcionariosLogin_cpf, cargos_codigo, data
            FROM $this->tabela
            WHERE administrativos_funcionarios_funcionariosLogin_cpf =  ?
            ORDER BY data
        ";
    ...
    public function excluirFuncionario($p) {
        $strSQL = "
            DELETE
            FROM $this->tabela
            WHERE administrativos_funcionarios_funcionariosLogin_cpf = ?
    ...
    public function existeFuncionario($p) {
        $strSQL = "
            SELECT *
            FROM $this->tabela
            WHERE administrativos_funcionarios_funcionariosLogin_cpf =  ?
