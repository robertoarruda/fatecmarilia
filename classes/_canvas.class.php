...
// fim redimensiona
    ...
    /**
     * Redimensiona imagem como preenchendo todo espaço especificado
     * (sem distorcer as dimensões originais da imagem)
     * @return void
     * @author Roberto Arruda <robertoadearruda@gmail.com>
     * */
    private function redimensionaPreencheTudo() {
        ...
        $dif_w = ceil($this->largura / $fator);
        $dif_h = ceil($this->altura / $fator);

        $dif_x = (($dif_w > $this->nova_largura) ? $this->nova_largura - $dif_w : $dif_w - $this->nova_largura) / 2;
        $dif_y = (($dif_h > $this->nova_altura) ? $this->nova_altura - $dif_h : $dif_h - $this->nova_altura) / 2;
        $this->img_temp = imagecreatetruecolor($this->nova_largura, $this->nova_altura);
        imagecopyresampled($this->img_temp, $this->img, $dif_x, $dif_y, 0, 0, $dif_w, $dif_h + 1, $this->largura, $this->altura);
