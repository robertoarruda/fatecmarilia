...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $objCurso->objetivo = returnPost('objetivo');
    $objCurso->publicoAlvo = returnPost('publicoAlvo');
    $objCurso->quadroDeDisciplinas = returnPost('quadroDeDisciplinas');
    $objCurso->duracao = returnPost('duracao');
    $objCurso->instituicao_codigo->codigo = returnPost('instituicao_codigo');

    $dir = "/$tipoUrl/cursos/pos-graduacao/";
    $erro = "
        <script>
        $.showMsg({
...
$lato = ($objCurso->tipo == '1') ? ' selected="selected"' : '';
$stricto = ($objCurso->tipo == '2') ? ' selected="selected"' : '';
...
$str = <<<STR
        ...
        >
            ...
            <option value="1"$lato>Lato Sensu</option>
            <option value="2"$stricto>Stricto Sensu</option>
        </select>
        ...
        <textarea id="objetivo" name="objetivo">$objCurso->objetivo</textarea>
        <label for="publicoAlvo">Público Alvo</label>
        <textarea id="publicoAlvo" name="publicoAlvo">$objCurso->publicoAlvo</textarea>
        <label for="quadroDeDisciplinas">Quadro de Disciplinas</label>
        <textarea id="quadroDeDisciplinas" name="quadroDeDisciplinas">$objCurso->quadroDeDisciplinas</textarea>
...
<html lang="pt-br">
    <head>
        ...
        <script src="/js/scripts/_cursoPos-form.js"></script>
            ...
            <article id="article">
                <div class="enquadramento1000px">
                    ...
                    <article class="boxForm">
                        <h1>Cadastro de Curso de Pós-graduação</h1>
                        <form method="post">
                            <?php print $str; ?>
                        </form>
                    </article>
